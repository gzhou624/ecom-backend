...
public class LoginService extends JFrame implements ActionListener {
	...
	public LoginService() {
		...
		title = new JLabel("Grace's ecom backend");
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(buttonExit))
			...
			new SignupService().setVisible(true);
		}
		else if (ae.getSource().equals(buttonLogin)) {
			int status = User.checkStatus(usernameTF.getText(), passwordF.getText());
			if (status == 0) {
				EmployeeService ea = new EmployeeService(usernameTF.getText());
				ea.setVisible(true);
				this.setVisible(false);
			}
			else if (status == 1) {
				CustomerService ca = new CustomerService(usernameTF.getText());
				ca.setVisible(true);
				this.setVisible(false);
			}
			else {
